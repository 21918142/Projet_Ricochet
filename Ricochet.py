...
def grid():
    ...
    for i in range(0, height, side):
        for j in range(0, width, side):
            color = "#D0CFD5"
            canvas.create_rectangle(i, j, i+side,
                                    j+side, fill=color, activefill="#928A99")
    for i in range(side*7, side*9, side):
        for j in range(side*7, side*9, side):
            color = "black"
            rectancle = canvas.create_rectangle(i, j, i+side, j+side,
                                                fill=color,
                                                activefill="#928A99")
...
def show_walls():
    ...
    for i in range(len(table)):
        for j in range(len(table[i])):
            if table[i][j] == "2":  # -
                line1 = canvas.create_line(i//2*side, j*side,
                                           (i//2+1)*side, j*side,
                                           fill="black", width=5)
                obstacle.append(line1)

            if table[i][j] == "1":  # |
                line2 = canvas.create_line(i//2*side, j*side,
                                           i//2*side, (j+1)*side,
                                           fill="black", width=5)
...
def show_robots():
    ...
    x = int(pos_robot[0][0])
    y = int(pos_robot[0][1])
    blue = canvas.create_oval(x*side+5, y*side+5,
                              x*side+35, y*side+35, fill="blue")
    x1, y1, x2, y2 = canvas.coords(blue)
    coord_robotb.append([x1, y1, x2, y2, 0])

    x = int(pos_robot[1][0])
    y = int(pos_robot[1][1])
    red = canvas.create_oval(x*side+5, y*side+5,
                             x*side+35, y*side+35, fill="red")
    u1, v1, u2, v2 = canvas.coords(red)
    coord_robotr.append([u1, v1, u2, v2, 1])

    x = int(pos_robot[2][0])
    y = int(pos_robot[2][1])
    green = canvas.create_oval(x*side+5, y*side+5,
                               x*side+35, y*side+35, fill="green")
    i1, j1, i2, j2 = canvas.coords(green)
    coord_robotg.append([i1, j1, i2, j2, 2])

    x = int(pos_robot[3][0])
    y = int(pos_robot[3][1])
    yellow = canvas.create_oval(x*side+5, y*side+5,
                                x*side+35, y*side+35, fill="yellow")
...
def show_target():
    ...
    x = int(target[0])
    y = int(target[1])
    target_b = canvas.create_rectangle(x*side+10, y*side+10,
                                       x*side+30, y*side+30, fill="blue")
    x = int(pos_robot[0][0])+2
    y = int(pos_robot[0][1])+1
    target_r = canvas.create_rectangle(x*side+10, y*side+10,
                                       x*side+30, y*side+30, fill="red")
    x = int(pos_robot[0][0])
    y = int(pos_robot[0][1])-10
    target_y = canvas.create_rectangle(x*side+10, y*side+10,
                                       x*side+30, y*side+30, fill="yellow")
    x = int(pos_robot[0][0])+9
    y = int(pos_robot[0][1])-10
    target_g = canvas.create_rectangle(x*side+10, y*side+10,
                                       x*side+30, y*side+30, fill="green")
...
def collision():
    ...
    if coord[0]//40 == b[0]//40 and coord[1]//40 == b[1]//40:
        canvas.coords(pos_target[bot], (16 + bot)*side+10, 15*side+10,
                                       (16 + bot)*side+30, 15*side+30)
        win()
        continues()
...
def keyboard(event):
    """Permet d'effectuer les déplacemenets
       du robot en utilisant les flèches du clavier"""
...
def sauvegarde():
    ...
    with open("saverobot", "wb") as f:
        ...
        bt = canvas.coords(pos_target[0])
        rt = canvas.coords(pos_target[1])
